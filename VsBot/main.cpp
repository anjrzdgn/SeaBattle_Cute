...
void choseDirection(QChar _table[10][10], int randX, int randY)
{
    ...
    while (count < 10) {
        ...
        {
            qDebug() << "saaaalaaaam7";
            if (exactDirection == "Left" /*&& randY > 0 && _table[randX][randY - 1] != 'W'*/)
            {
                qDebug() << "saaaalaaaam8";
                attackAgain(_table, randX, randY - 1);
            }
            else if (exactDirection == "Right" /*&& randY < 9 && _table[randX][randY + 1] != 'W'*/)
            {
                qDebug() << "saaaalaaaam9";
                attackAgain(_table, randX, randY + 1);
            }
            else if (exactDirection == "Up"/* && randX > 0 && _table[randX - 1][randY] != 'W'*/)
            {
                qDebug() << "saaaalaaaam10";

                attackAgain(_table, randX - 1, randY);
            }
            else if (exactDirection == "Down" /*&& randX < 9 && _table[randX + 1][randY] != 'W'*/)
            {
                qDebug() << "saaaalaaaam11";
